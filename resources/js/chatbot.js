...
`;
        ...
        document.getElementById("chatbot-greeting").style.bottom =
            _initBase + 68 + "px";
        document.getElementById("chatbot-container").style.bottom =
            _initBase + 70 + "px";
        ...
        chatIcon.addEventListener("click", () => {
            ...
            if (isOpen) {
                ...
                if (
                    promo &&
                    promo.classList.contains("is-mounted") &&
                    promo.classList.contains("is-hidden-scroll")
                ) {
        ...
        function addMessage(sender, text) {
            ...
            const maxWidth =
                window.innerWidth > 768 ? 500 : window.innerWidth * 0.9;
        ...
        async function sendMessage() {
            ...
            function highlightPromo(replyText) {
                ...
                if (
                    priceKeywords.some((kw) => text.toLowerCase().includes(kw))
                ) {
                    return replyText.replace(
                        /(Grand Opening.*?OFF)/gi,
                        "🔥 $1 🔥",
                    );
                }
                return replyText;
            ...
            const streamingDisabled =
                sessionStorage.getItem("rc_stream_ok") === "0";
            ...
            try {
                ...
                if (err && err.code === "NO_CHUNKS") {
                    ...
                    console.warn(
                        "[RielBot] Streaming watchdog tripped - falling back to blocking JSON.",
                    );
                    try {
                        await sendBlocking(text, botMsgDiv, highlightPromo);
                    } catch (err2) {
        ...
        function updateChatbotOffset(promoUp) {
            ...
            const offset = promoUp
                ? (promo ? promo.offsetHeight : 0) + 12
                : base;
