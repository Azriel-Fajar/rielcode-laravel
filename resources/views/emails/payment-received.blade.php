...
<head>
<meta charset="UTF-8">
<title>Payment Received | Rielcode</title>
</head>
...
<table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.08);">
    <tr>
        <td style="background:#2d4a3a;padding:24px 32px;text-align:center;">
            <p style="margin:0;font-size:20px;font-weight:700;color:#f4f1ea;">Rielcode</p>
        </td>
    </tr>
    <tr>
        <td style="padding:32px;">
            ...
            <p style="margin:0 0 16px;font-size:15px;color:#333;line-height:1.6;">
                We have received your <strong>{{ $payment->stageLabel() }}</strong> payment of <strong>{{ $payment->amountFormatted() }}</strong>. Thank you!
            </p>

            @if($payment->stage === 'deposit')
                <p style="margin:0 0 16px;font-size:15px;color:#333;line-height:1.6;">
                    Your project is now officially <strong>in progress</strong>. We will be in touch with updates along the way.
                </p>
            @else
                <p style="margin:0 0 16px;font-size:15px;color:#333;line-height:1.6;">
                    Your project is now <strong>complete</strong>. Admin credentials and handover details will be sent separately.
                </p>
            @endif
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8faf8;border:1px solid #d1e7d9;border-radius:8px;padding:16px 20px;margin:20px 0;">
                ...
                <tr><td style="font-size:14px;color:#111;padding-bottom:4px;"><strong>Paid at:</strong> {{ $payment->paid_at?->format('d M Y H:i') ?? 'Confirmed' }}</td></tr>
            </table>
    ...
    <tr>
        <td style="background:#f8fafc;padding:16px 32px;text-align:center;border-top:1px solid #e2e8f0;">
            <p style="margin:0;font-size:11px;color:#94a3b8;">Sent by Rielcode. If this is unexpected, please contact us.</p>
        </td>
    </tr>
</table>
</td></tr>
